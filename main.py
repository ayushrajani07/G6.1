...
# --- ⬇️ CORRECTED IMPORT PATH ⬇️ ---
# The market_hours module is inside the 'utils' sub-package
from .utils.market_hours import is_market_open, get_next_market_open, sleep_until_market_open
# --- ⬆️ CORRECTED IMPORT PATH ⬆️ ---
...
from .utils.resilience import retry, fallback, HealthCheck
...
def parse_arguments():
    ...
    parser.add_argument('--data-dir', default='data/g6_data',
                        help='Path to data directory (default: data/g6_data)')
...
def initialize_providers(config, debug_midcpnifty=False):
    ...
    try:
        if provider_type == 'kite':
            ...
            token_path = provider_config.get('token_path', 'config/kite_token.json')
            ...
            from .broker.dummy_provider import DummyProvider
...
def initialize_storage(config):
    ...
    # Initialize CSV sink
    data_dir = config.get('data_dir', 'data/g6_data')
    ...
    while app_state.running:
        try:
            ...
            if not is_market_open(market_type="equity", session_type="regular", reference_time=next_collection_time):
                ...
                from .utils.market_hours import DEFAULT_MARKET_HOURS
...
def main():
    ...
    try:
        ...
        app_state.providers = initialize_providers(config)
        ...
        try:
            ...
            app_state.metrics = PrometheusMetrics()
...
if __name__ == "__main__":
    # This block allows running the main application directly as a script
    # It sets up the package context correctly.
    # Note: It's still recommended to run via `python -m src.main`
