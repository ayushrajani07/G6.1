...
class OptionGreeks:
    ...
    def _calculate_dte(expiry_date: Union[date, datetime], current_date: Optional[Union[date, datetime]] = None) -> float:
        ...
        # Add partial day
        if isinstance(current_date, datetime):
            seconds_today = (current_date - current_date.replace(hour=0, minute=0, second=0)).total_seconds()
            days += seconds_today / 86400
    ...
    def black_scholes(
        ...
        T: Union[float, datetime, date],  # Time to expiry in years or expiry date
    ...
    def implied_volatility(
        ...
        if market_price <= 0.01:
            return 0.01  # Minimum IV to avoid division by zero issues
