...
class Providers:
    ...
    def get_index_data(self, index_symbol):
        ...
        try:
            ...
            # Get quote from primary provider (includes OHLC)
            quotes = self.primary_provider.get_quote(instruments)
            ...
            ltp_data = self.primary_provider.get_ltp(instruments)
    ...
    def resolve_expiry(self, index_symbol, expiry_rule):
        ...
        try:
            if hasattr(self.primary_provider, 'resolve_expiry'):
                return self.primary_provider.resolve_expiry(index_symbol, expiry_rule)
    ...
    def get_option_instruments(self, index_symbol, expiry_date, strikes):
        ...
        try:
            ...
            if hasattr(self.primary_provider, 'get_option_instruments'):
                instruments = self.primary_provider.get_option_instruments(index_symbol, expiry_date, strikes)
                if instruments:
                    return instruments
            
            # Fallback to option_instruments if available
            if hasattr(self.primary_provider, 'option_instruments'):
                instruments = self.primary_provider.option_instruments(index_symbol, expiry_date, strikes)
                if instruments:
                    return instruments
    ...
    def get_quote(self, instruments):
        ...
        try:
            if hasattr(self.primary_provider, 'get_quote'):
                return self.primary_provider.get_quote(instruments)
            return {}
        except Exception as e:
    ...
    def enrich_with_quotes(self, instruments):
        ...
        try:
            ...
            quotes = self.primary_provider.get_quote(quote_instruments)
