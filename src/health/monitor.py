...
class HealthMonitor:
    ...
    def start(self):
        ...
        self.logger.info(f"Health monitoring started with {len(self.components)} components")
    ...
    def _check_all_components(self):
        ...
        for name, data in self.components.items():
            try:
                ...
                if status != 'healthy':
                    self.logger.warning(f"Component {name} health check: {status} - {message}")
                else:
                    self.logger.debug(f"Component {name} health check: {status}")
        ...
        for check in self.health_checks:
            try:
                ...
                if check['status'] != 'healthy':
                    self.logger.warning(f"Health check {check['name']}: {check['status']} - {check['message']}")
                else:
