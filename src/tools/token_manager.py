...
def validate_token(api_key, access_token):
    ...
    try:
        ...
        logger.info(f"Token is valid. Logged in as: {profile.get('user_name', 'Unknown')}")
        return True
...
def run_main_application():
    """Run the main G6 Platform application."""
    try:
        logger.info("Starting G6 Platform...")
        
        # Method 1: Import and run main directly
        try:
            from src.main import main
            return main()
        except ImportError:
            # Method 2: Use subprocess as fallback
            logger.info("Launching main application as subprocess...")
...
def flask_login_server(api_key, api_secret, auto_run_app=True):
    ...
    # Get login URL (with or without redirect_uri depending on API version)
    try:
        # Try with redirect_uri first (newer API versions)
        login_url = kite.login_url(redirect_uri=redirect_uri)
    except TypeError:
        # Fallback to default login URL (older API versions)
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='G6 Platform Token Manager')
    parser.add_argument('--no-autorun', action='store_true', help='Do not automatically run main application')
    args = parser.parse_args()
    
    auto_run_app = not args.no_autorun
