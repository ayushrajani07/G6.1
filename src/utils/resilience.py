...
def retry(
    ...
    jitter: bool = True,
    exceptions: Union[List[Exception], Tuple[Exception, ...]] = (Exception,)
):
    """
...
def fallback(default_value: Any, exceptions: Union[List[Exception], Tuple[Exception, ...]] = (Exception,)):
    """
...
class HealthCheck:
    ...
    @staticmethod
    def check_provider(provider, method_name='get_ltp', args=None, kwargs=None, timeout=5):
        """
        ...
        try:
            ...
            return {
                'status': 'unhealthy',
                'message': f"Provider health check timed out after {timeout}s"
            }
        except Exception as e:
            return {
                'status': 'unhealthy',
    ...
    @staticmethod
    def check_storage(storage, method_name='check_health', timeout=5):
        """
        ...
        try:
            ...
            return {
                'status': 'unhealthy',
                'message': f"Storage health check timed out after {timeout}s"
            }
        except Exception as e:
            return {
                'status': 'unhealthy',
